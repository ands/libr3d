...
static inline int int_clamp(int value, int min, int max) { return value < min ? min : (value < max ? value : max); }

static inline float float_min(float a, float b) { return a < b ? a : b; }
static inline float float_max(float a, float b) { return a < b ? b : a; }
static inline float float_clamp(float value, float min, float max) { return value < min ? min : (value < max ? value : max); }

static inline vec2_t vec2(float x, float y) { vec2_t r = { x, y }; return r; }
static inline vec2_t vec2_add(vec2_t v0, vec2_t v1) { vec2_t r = { v0.x + v1.x, v0.y + v1.y }; return r; }
static inline vec2_t vec2_sub(vec2_t v0, vec2_t v1) { vec2_t r = { v0.x - v1.x, v0.y - v1.y }; return r; }
static inline vec2_t vec2_mul(vec2_t v, float f) { vec2_t r = { v.x * f, v.y * f }; return r; }
static inline vec2_t vec2_div(vec2_t v, float f) { float fi = 1.0f / f; vec2_t r = { v.x * fi, v.y * fi }; return r; }
static inline float vec2_dot(vec2_t v0, vec2_t v1) { return v0.x * v1.x + v0.y * v1.y; }
static inline float vec2_cross(vec2_t v0, vec2_t v1) { return v0.x * v1.y - v0.y * v1.x; }
static inline float vec2_length(vec2_t v) { return sqrtf(v.x * v.x + v.y * v.y); }
static inline vec2_t vec2_normalize(vec2_t v) { float fi = 1.0f / sqrtf(v.x * v.x + v.y * v.y); vec2_t r = { v.x * fi, v.y * fi }; return r; }

static inline vec3_t vec3(float x, float y, float z) { vec3_t r = { x, y, z }; return r; }
static inline vec3_t vec3_2(vec2_t xy, float z) { vec3_t r = { xy.x, xy.y, z }; return r; }
static inline vec3_t vec3_add(vec3_t v0, vec3_t v1) { vec3_t r = { v0.x + v1.x, v0.y + v1.y, v0.z + v1.z }; return r; }
static inline vec3_t vec3_sub(vec3_t v0, vec3_t v1) { vec3_t r = { v0.x - v1.x, v0.y - v1.y, v0.z - v1.z }; return r; }
static inline vec3_t vec3_mul(vec3_t v, float f) { vec3_t r = { v.x * f, v.y * f, v.z * f }; return r; }
static inline vec3_t vec3_div(vec3_t v, float f) { float fi = 1.0f / f; vec3_t r = { v.x * fi, v.y * fi, v.z * fi }; return r; }
static inline float vec3_dot(vec3_t v0, vec3_t v1) { return v0.x * v1.x + v0.y * v1.y + v0.z * v1.z; }
static inline vec3_t vec3_cross(vec3_t v0, vec3_t v1) { vec3_t r = { v0.y * v1.z - v0.z * v1.y, v0.z * v1.x - v0.x * v1.z, v0.x * v1.y - v0.y * v1.x }; return r; }
static inline float vec3_length(vec3_t v) { return sqrtf(v.x * v.x + v.y * v.y + v.z * v.z); }
static inline vec3_t vec3_normalize(vec3_t v) { float fi = 1.0f / sqrtf(v.x * v.x + v.y * v.y + v.z * v.z); vec3_t r = { v.x * fi, v.y * fi, v.z * fi }; return r; }

static inline vec4_t vec4(float x, float y, float z, float w) { vec4_t r = { x, y, z, w }; return r; }
static inline vec4_t vec4_2(vec2_t xy, float z, float w) { vec4_t r = { xy.x, xy.y, z, w }; return r; }
static inline vec4_t vec4_3(vec3_t xyz, float w) { vec4_t r = { xyz.x, xyz.y, xyz.z, w }; return r; }
static inline vec4_t vec4_add(vec4_t v0, vec4_t v1) { vec4_t r = { v0.x + v1.x, v0.y + v1.y, v0.z + v1.z, v0.w + v1.w }; return r; }
static inline vec4_t vec4_sub(vec4_t v0, vec4_t v1) { vec4_t r = { v0.x - v1.x, v0.y - v1.y, v0.z - v1.z, v0.w - v1.w }; return r; }
static inline vec4_t vec4_mul(vec4_t v, float f) { vec4_t r = { v.x * f, v.y * f, v.z * f, v.w * f }; return r; }
static inline vec4_t vec4_div(vec4_t v, float f) { float fi = 1.0f / f; vec4_t r = { v.x * fi, v.y * fi, v.z * fi, v.w * fi }; return r; }
static inline float vec4_dot(vec4_t v0, vec4_t v1) { return v0.x * v1.x + v0.y * v1.y + v0.z * v1.z + v0.w * v1.w; }
static inline vec4_t vec4_cross(vec4_t v0, vec4_t v1) { return vec4_3(vec3_cross(v0.xyz, v1.xyz), (v0.w + v1.w) * 0.5f); } // USUALLY FAIL
static inline float vec4_length(vec4_t v) { return sqrtf(v.x * v.x + v.y * v.y + v.z * v.z + v.w * v.w); }
static inline vec4_t vec4_normalize(vec4_t v) { float fi = 1.0f / sqrtf(v.x * v.x + v.y * v.y + v.z * v.z + v.w * v.w); vec4_t r = { v.x * fi, v.y * fi, v.z * fi, v.w * fi }; return r; }
...
static inline mat4_t mat4_rotation(float angle, vec3_t axis)
{
	...
	c *= -1.0f; s *= -1.0f; // we're 180° off. correct this
#else
